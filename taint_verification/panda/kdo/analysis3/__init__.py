...
def __replay(rootfs, kernel, record, ignored_addresses, func_map, symbol_map,
	...
	def on_ret(cpu, addr):
		...
		if addr == kmem_cache_alloc_addr or addr == kmem_cache_alloc_node_addr or addr == kmem_cache_alloc_lru_addr or addr == __kasan_kmalloc_addr or addr == __kasan_slab_alloc_addr:
			...
			if entry is None:
				print(f'[analysis3] on_ret: no callstack entry for rsp-8={hex(rsp-8)}, skipping')
				return
			cachep = entry["arg"]
			if not cachep:
				print(f'[analysis3] on_ret: cachep is NULL, skipping allocation record')
				return
			try:
				...
				if not cachename_p:
					print(f'[analysis3] on_ret: cachename_p is NULL for cachep={hex(cachep)}, using <unknown>')
					cachename = "<unknown>"
				else:
					cachename = panda.virtual_memory_read(cpu, cachename_p, 40, fmt="str")
			except ValueError as e:
				print(f'[analysis3] on_ret: failed to read kmem_cache fields at cachep={hex(cachep)}: {e}')
				size = 0
				cachename = "<unknown>"
