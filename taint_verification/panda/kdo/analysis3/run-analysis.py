...
#           analysis3.json           — output written here
...
def doit(repro):
    ...
    print(f"[run-analysis3] starting {repro_id!r} (kernel={kernel_name!r})")
    ...
    if share_path:
        ...
        if os.path.isfile(repro_bin):
            ...
            print(f"[run-analysis3] using repro binary: {repro_bin}")
        else:
            print(f"[run-analysis3] warning: repro binary not found at {repro_bin}")

    # Recording files live in the repro output dir; replay must run from there.
    os.chdir(repro_out)

    analysis3.replay(rootfs, kernel, sink_call_id=sink_call_id,
...
def main() -> None:
    opts = argparse.ArgumentParser(
            description='Run analysis3 replay over recordings from run-repros.py')
    ...
    opts.add_argument('--rerun', action='store_true',
            help='only run entries whose analysis3.json is missing')
    ...
    analysis3.update_config(share_path=args.share_path)
    ...
    if args.repro_id:
        ...
        torun = [
            r for r in crashed
            if not os.path.isfile(os.path.join(OUT_BASE, r['id'], 'analysis3.json'))
        ]
        for r in torun:
            print(f"[run-analysis3] queued (missing analysis3.json): {r['id']}")
    else:
        torun = crashed

    print(f"[run-analysis3] {len(torun)} repro(s) to analyse (parallelism={args.npar})")
