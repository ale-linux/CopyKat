...
def __replay(rootfs, kernel, record, _ignored_addresses, _func_map, symbol_map, enable_logging):
	...
	def on_call(cpu, addr):
		...
		if kdo_store_cb_addr is not None and addr == kdo_store_cb_addr:
			# void kdo_store_callback(void *ptr, int len)
			# x86_64 SysV ABI: arg0=rdi (ptr), arg1=rsi (len)
			...
			print(f'[analysis1] kdo_store_callback(ptr=0x{ptr:x}, len={length})')

			# Label each byte [ptr, ptr+length-1] with a unique taint label
			enable_taint()
	...
	def on_sys_execve_enter(cpu, pc, fname_ptr, argv_ptr, envp):
		...
		print(f"[analysis1] repro execve detected: {fname} — enabling on_call hooks")
